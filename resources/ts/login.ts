...
(($, $$, _, __) => {
    ...
    const isMobileOTPAvailable:boolean = identityInput.dataset.mobileOtp == 'true';
    ...
    showOtpButton?.addEventListener('click', async () => {
        ...
        if(!isMobileOTPAvailable && isMobile(identityInput.value)) {
            sweetAlert.fire('Mobile OTP Not Available', 'Mobile OTP service is not available at this time!<br />Our team is looking for a solution, meanwhile, use your email id for OTP.', 'warning').then(() => identityInput.focus());
            return;
        }
        ...
        try {
            ...
            console.error(err);
            sweetAlert.fire('Error!', err.message, 'error');
        }
