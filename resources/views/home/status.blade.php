...
@section('content')
                            ...
                            <tr bgcolor="#E36E2C">
                                <td colspan="6">
                                    ...
                                    @if ($application->status->id == '302')
                                        <div align="center" class="style1">
                                            <h5>Click <a
                                                    href="{{ route('application.newedit', [
                                    ...
                                    <div class="card z-index-0 fadeIn3 fadeInBottom">
                                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                            <div class="bg-gradient-primary1 shadow-primary border-radius-lg py-3 pe-1">
                                                <h3 class="text-white font-weight-bolder text-center mt-2 mb-0">MUKHYA
                                                    MANTRI SWAVALAMBAN YOJANA</h3>
                                                <h4 class="text-white font-weight-bolder text-center mt-2 mb-0">GOVT. OF
                                                    HIMACHAL PRADESH</h4>
                                                <h5 class="text-white font-weight-bolder text-center mt-2 mb-0">Applicant
                                                    Sign in</h5>
                                            </div>
                                        </div>
                                        <div class="card-body">
                                            ...
                                            @endif
                                            <form data-key="{{ request()->session()->get('_key') }}" method="POST"
                                                action="{{ route('login.request') }}">
                                                @csrf
                                                <div class="form-group">
                                                    <label
                                                        for="identity">{{ __('Enter you Email / Mobile / Application ID') }}</label>
                                                    <input id="identity"
                                                        data-mobile-otp="{{ setting('is_mobile_otp_available', null, false) ? 'true' : 'false' }}"
                                                        type="text"
                                                        class="form-control @error('identity') is-invalid @enderror @error('mobile') is-invalid @enderror"
                                                        name="identity" placeholder="Email,Phone,Application No."
                                                        autocomplete="username" value="{{ old('identity') }}" required
                                                        autofocus />
                                                    @error('identity')
                                                ...
                                                <div class="d-none" id="passwordBox">
                                                    ...
                                                    <div
                                                        ...
                                                        <label class="form-check-label mb-0 ms-3" for="showPassword">Show
                                                            Password</label>
                                                    </div>
                                                    @error('password')
                                                        <span class="invalid-feedback" role="alert">
                                                ...
                                                <div class="form-group d-none" id="otpBox">
                                                    ...
                                                    <input id="otpCode" type="number" min="100000" max="999999"
                                                        autocomplete="nickname"
                                                        class="form-control @error('otpCode') is-invalid @enderror"
                                                        name="otpCode" />
                                                    @error('otpCode')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                ...
                                                <div class="text-center" id="optionButtonBox">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <button type="button"
                                                                style=" background: #e36e2c; color:white;width: 80%;"
                                                                id="showOtpButton"
                                                                class="btn bg-gradient-primary1 w-80 my-4 mb-2"
                                                                data-url="{{ route('otp.request') }}">
                                                                <em class="fa-sharp fa-solid fa-comment-sms"></em>
                                                                {{ __('Generate OTP') }}</button>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-12">
                                                        ...
                                                        <div class="input-group input-group-outline mb-3 d-none">
                                                            <label for="captcha"
                                                                class="form-label">{{ __('Security Code') }}</label>
                                                            <input id="captcha" type="text" required="required"
                                                                minlength="{{ config('captcha.default.length') }}"
                                                                maxlength="{{ config('captcha.default.length') }}"
                                                ...
                                                <div class="text-center d-none">
                                                    <button type="submit"
                                                        style=" background: #e36e2c; color:white; width: 80%;"
                                                        class="btn bg-gradient-primary1  my-4 mb-2">Sign
                                                        in</button><br />
                                                    <a href="{{ route('login.applicant') }}"
                                                        class="btn bg-gradient-secondary w-100 my-4 mb-2">Reset</a>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
    ...
    @if (count($applications))
        <div class="text-center">
            <h5 style="background-color: rgb(255, 138, 48); padding: 10px; color: white">{{ $application->status->name }}
            </h5>
            @if ($application->status->id == 306)
                <p>Your application is submitted and pending at DIC. You will get notifications for further actions.</br>
                    आपका आवेदन सबमिट किया गया है और DIC में लंबित है। आपको आगे की कार्रवाई के लिए सूचनाएं मिलेंगी।</p>
            @endif
            @if ($application->status->id == 302)
                <p>Your application is not completed yet please click on below button to complete it.</br> आपका आवेदन अभी
                    पूरा नहीं हुआ है, कृपया नीचे दिए गए बटन पर क्लिक करके इसे पूरा करें।</p>
            @endif
            ...
            @if ($application->status->id == 302 || $application->status->id == 305)
                <a href="{{ route('application.newedit', ['application' => $application]) }}">
                    <button class="btn btn-primary">{{ $application->status->id == 302 ? 'Complete' : 'Edit' }} Your
                        Application</button>
                </a>
            @endif
        </div>
...
$(document).ready(function() {
    ...
    function showOtpInput() {
        ...
        $('.send-otp-btn').hide();
            ...
            const $captchaButton = $('#captchaRefreshButton');
            const captchaUrl = $captchaButton.prev().attr('src');
            $captchaButton.click(() => $captchaButton.prev().attr('src',
                `${captchaUrl}?_t=${(new Date()).getTime()}`));
        });
    </script>
@endsection
